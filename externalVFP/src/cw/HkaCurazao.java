 ...
 * @author pmoya
 */
public class HkaCurazao extends HkaBaseRaiz implements IFiscalAccionable{
 public HkaCurazao()
  {   super();
      this.setLocaleCurrencySimbol("NAf");
    ...
    public void setCommad(Command pCmd) throws DeviceException {
         ...
         if(pCmd.isValido(pCmd._contentResource))
        {
         switch (pCmd._acction)
         {
            ...
            case "printReportZDate":
                this.BeginFiscalDocument("10");
    ...
    public int BeginFiscalDocument(String pType) {
        ...
        if(pType != null)
        {
         ...
         { 
             ...
             _myTicket.setNameType("FISCAL INVOICE");
              this._state = STATE_TRANSACCTION;
         }
         else if(pType.equals("2"))
         {
              _myTicket.setTicketType(2);
             _myTicket.setNameType("FISCAL INVOICE WITH FISCAL VALUE");
              this._state = STATE_TRANSACCTION;
         }
         else if(pType.equals("3"))
         {
             _myTicket.setTicketType(3);
             _myTicket.setNameType("CREDIT NOTE FOR INVOICE WHIT FISCAL VALUE");
              this._state = STATE_TRANSACCTION;
         }
    ...
    public int VoidFiscalDocument() {
        this.setTypeDisplay(PRINTABLE);
         List<String> lineNotFiscal = new ArrayList<>();
                 ...
                 lineNotFiscal.add("DOCUMENT CANCELLED NRO. " + this._dataLogicSales.getNewTicket_Id(this._myTicket.getTicketType()));
                 lineNotFiscal.add("BY USER");
                 this._myTicket.setNotFiscalLines(lineNotFiscal);
         return RESPONSE_OK;
    }
        ...
        this.BeginFiscalDocument("9");
                 ...
                 Flags[] listFlags = fl.listFlags(this._myTicket.getEquipmet().getId());
                  int j = 0;
                        ...
                        {
                          lineNotFiscal.add("TAX RATE " + t.getId() + " (" + Formats.DOUBLE.formatValue(t.getRate()*100) +"%)");
                        }
